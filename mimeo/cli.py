...
def list_sites(config: Path | None, format: str, health: bool, fix: bool, dns_check: bool) -> None:
...
def registrar_list(config: Path | None, output_format: str, with_dns: bool, workers: int) -> None:
    ...
    try:
        ...
        # Assign stable indices for ordering after concurrent enrichment
        ...
        def _enrich(idx: int, domain_info: Dict[str, Any]) -> Dict[str, Any]:
            ...
            if with_dns:
                with PorkbunDNSProvider(cfg.porkbun_api_key, cfg.porkbun_secret) as dns:
                    result["dns_records"] = dns._get_domain_records(domain_name)
            else:
                result["dns_records"] = []
