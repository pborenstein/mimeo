...
**Status**: Active (Phase 8) — Stages 5A (`create` absorbs `template apply`), 5B (`status` absorbs `list`/`registrar list`/`dns show`/`dns check`), and 5C (`sync` absorbs `dns repair`/`fix https`) are implemented. Full task checklist in IMPLEMENTATION.md Phase 8 Stage 5.
